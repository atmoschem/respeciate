...
#' \code{name}d species.
#' @param parent (for \code{pls_fit_parent} only) a data.frame of measurements
...
#' @param start,lower,upper all numeric, the start value and lower and upper
#' boundaries when fitting a parameter.
#' @param species,profile (for \code{pls_plot}s only) numeric or character
...
#' as the profile code. Both can be concatenation to product multiple plots and
#' the special case \code{-1} is a short cut to all species or profiles,
#' respectively.
...
pls_plot <- function(pls, species=1, type=1,...){
...
pls_plot_species <- function(pls, species, type=1, ...){
...
pls_plot_profile <- function(pls, profile, log=FALSE, ...){
...
#    col <- rep_len(col, nx)
  ...
  for (i in 1L:nx) {
    ...
    if(!as.character(labels[i]) == "[hide]"){
      ...
      polygon(c(P$x, rev(P$x*0.5)), c(P$y, rev(P$y*0.5)), density = density[i], angle = angle[i],
              border = border[i], col = col[i], lty = lty[i])
      P <- t2xy(mean(x[i + 0:1]))
      lab <- as.character(labels[i])
      if (!is.na(lab) && nzchar(lab)) {
        # 1.1 and 1.2 are the extenders when moving labels way from
        # the pie plot itself
